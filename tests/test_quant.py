...
test_bundles = [
    ("empty", [("chr1", "+", {"gn": "-", "gf": "-"})], None),
    (
        "uniq",
        [("chr", "+", {"CB": "ACGTACGT", "MI": "AAAAC", "gn": "A", "gf": "N"})],
...
def test_gene_selection():
    ...
    assert counter.select_gene(
        "chr1", "+", {"gn": "A,B", "gf": "C|U|I,i|n", "AS": 90}
    ) == (
        "A",
        ["C"],
    )
    assert counter.select_gene(
        "chr1", "+", {"gn": "A,A,A", "gf": "C|I,I,U", "AS": 90}
    ) == (
        "A",
        ["C", "I", "U"],
    )
    assert counter.select_gene(
        "chr1", "+", {"gn": "A1,A2,A3", "gf": "C|I,I,U", "AS": 90}
    ) == (
        "A1",
        ["C"],
    )
